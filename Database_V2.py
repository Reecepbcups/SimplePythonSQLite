#!/usr/bin/python3
"""
    Updated for my course on Python at Capella University
    - 10/11/2021, Reece

    This is a module which is used to easily create, maintain, and change records
...
#DATABASE_NAME = 'Credentials.db' #":memory:" # if debugging
...
class DBFunctions:
    ...
    def getCustomRecords(self, colums=[]):
        with self.conn:
            self.c.execute(f"SELECT {' ,'.join(colums)} FROM {self.TableName}")
            #print(f"SELECT {', '.join(colums)} FROM {self.TableName}")

        return self.c.fetchall()

    def deleteLastRecord(self, key_ID):
        with self.conn:
            self.c.execute(f"SELECT * FROM {self.TableName} ORDER BY {key_ID} DESC limit 1")
            recordRemoved = self.c.fetchone()
            
        with self.conn:    
            self.c.execute(f"DELETE FROM {self.TableName} ORDER BY {key_ID} DESC limit 1")

        if DEBUGGING:
            DEBUG('deleteLastRecord', f'[-] {recordRemoved} removed')
